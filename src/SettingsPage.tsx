...
import { MODEL_LIST, DEFAULT_MODEL } from "./services/constants";
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} maxWidth="max-w-lg">
      <div className="p-6">
        ...
        {changes.length > 0 && (
          <div className="bg-slate-100 rounded-lg p-4 mb-6 max-h-64 overflow-y-auto">
            {changes.map((change, idx) => (
              <div key={idx} className="flex items-center justify-between py-2 text-sm border-b border-slate-200 last:border-0">
                <span className="text-slate-600">{change.label}</span>
                <span className="text-slate-400 text-right ml-4">
                  {change.current} → <span className="text-indigo-600">{change.new}</span>
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={onConfirm}
            className="flex-1 px-4 py-2 bg-amber-500 hover:bg-amber-500 text-white rounded-lg transition-colors duration-200"
          >
            Reset Settings
          </button>
        </div>
  ...
  return (
    <>
      <div className="animate-fade-in max-w-2xl mx-auto">
        ...
        <button
          onClick={resetAll}
          className="flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors duration-200 w-full sm:w-auto">
          <svg
            className="w-4 h-4"
            fill="none"
            viewBox="0 0 24 24"
        ...
        <div className="p-4 sm:p-6">
          <div className="flex items-start justify-between mb-6">
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-500">Configure spaced repetition behavior</p>
            </div>
            <button
              onClick={resetLearningPool}
              title="Reset Learning Pool to defaults"
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-md transition-colors duration-200"
            >
      ...
      <div className="mt-6 p-4 bg-slate-50 border border-slate-200 rounded-lg">
        <p className="text-sm text-slate-500">
          <span className="text-slate-600 font-medium">Preview:</span> Your quiz
          will have{" "}
...
function NumberSetting({
  ...
  return (
    <div className="flex items-center justify-between py-3 px-4 rounded-lg hover:bg-slate-100 transition-colors duration-200" role="group" aria-labelledby={labelId}>
      <div>
        <p id={labelId} className="text-slate-900 font-medium">{label}</p>
        <p id={descId} className="text-sm text-slate-500">{description}</p>
      </div>
      <div className="flex items-center gap-2">
        <button
          onClick={() => onChange(Math.max(min, value - 1))}
          className="w-10 h-10 sm:w-8 sm:h-8 flex items-center justify-center rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-900 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={value <= min}
        ...
        <input
          ...
          onBlur={handleBlur}
          className="w-14 h-8 text-center bg-slate-100 border border-slate-300 rounded-lg text-slate-900 font-medium text-lg focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
          aria-label={label}
  ...
  return (
    <div
      ...
      <div>
        <p
          id={labelId}
          className={`font-medium ${disabled ? "text-slate-500" : "text-slate-900"}`}>
          {label}
        </p>
        <p id={descId} className="text-sm text-slate-500">{description}</p>
      </div>
      <button
        ...
        aria-describedby={descId}
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-white ${
...
function SelectSetting({
  ...
  return (
    <div className="py-3 px-4 rounded-lg hover:bg-slate-100 transition-colors duration-200">
      <div className="flex items-center justify-between mb-2">
        ...
        <select
          ...
          className="bg-slate-100 border border-slate-300 text-slate-900 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 cursor-pointer">
          {options.map((option) => (
...
function ModelPicker({ value, onChange }: ModelPickerProps) {
  return (
    <div className="py-3 px-4 rounded-lg">
      <div className="flex items-center gap-2 mb-1">
        <p className="text-slate-900 font-medium">AI Model</p>
        <HelpIcon tooltip="The OpenAI model used to generate and grade questions. Pricier models generally produce higher-quality quizzes. Prices are per 1M tokens (input / output)." />
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="text-right shrink-0">
                ...
                <p className="text-[10px] text-slate-400">per 1M in / out</p>
              </div>
            </button>
          );
        })}
  ...
  return (
    <div className="py-3 px-4 rounded-lg hover:bg-slate-100 transition-colors duration-200">
      ...
      <textarea
        ...
        rows={3}
        className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 resize-none"
      />
    </div>
  );
}
  ...
  return (
    <div className="py-3 px-4 rounded-lg hover:bg-slate-100 transition-colors duration-200">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
        <div className="flex items-center gap-2">
          <span className="text-sm text-slate-500 w-8">Min</span>
          <button
            onClick={() => onMinChange(Math.max(absoluteMin, minValue - 1))}
            className="w-10 h-10 sm:w-7 sm:h-7 flex items-center justify-center rounded-md bg-slate-100 hover:bg-slate-200 text-slate-900 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <span className="w-10 h-8 sm:w-8 sm:h-7 flex items-center justify-center bg-slate-100 border border-slate-300 rounded-md text-slate-900 font-medium text-sm">
            {minValue}
          </span>
          <button
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-slate-500 w-8">Max</span>
          <button
...
function ExamplePreview({
  ...
  return (
    <div role="region" aria-label="Example question preview" className="mt-4 p-4 bg-slate-100 rounded-lg border border-slate-200">
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-medium text-slate-500">Example Preview</p>
      ...
      {/* Sample text */}
      <p className="text-xs text-slate-400 mb-3 italic leading-relaxed">
        "{SAMPLE_TEXT}"
      </p>
      ...
      <div className="bg-white rounded-lg p-3 border border-slate-200">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-emerald-600">{example.answers[0]}</span>
          </div>
          {allowMultipleCorrectAnswers && (
            <div className="flex items-center gap-2 text-sm">
              <span className="w-4 h-4 rounded-full border-2 border-emerald-500 flex items-center justify-center flex-shrink-0">
              ...
              <span className="text-emerald-600">Another correct answer</span>
            </div>
          )}
          <div className="flex items-center gap-2 text-sm">
            <span className="w-4 h-4 rounded-full border-2 border-slate-300 flex-shrink-0"></span>
            <span className="text-slate-500">{example.answers[1]}</span>
          </div>
        </div>
      </div>
