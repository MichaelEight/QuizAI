...
export function FilePreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in">
      ...
      {/* Modal */}
      <div className="relative bg-white border border-slate-200 rounded-xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col mx-4">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <div className="flex items-center gap-3">
            <svg
              className={`w-5 h-5 ${file.type === 'pdf' ? 'text-rose-600' : 'text-blue-600'}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <div>
              <h2 className="text-lg font-semibold text-slate-900">{file.name}</h2>
              <p className="text-sm text-slate-500">Preview and edit extracted text</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-hidden p-6">
          <textarea
            ...
            className="w-full h-full min-h-[300px] bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 transition-all duration-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 resize-none font-mono text-sm"
