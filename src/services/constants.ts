...
export class Instructions {
  ...
  private static readonly ANSWER_BALANCE = `
    ...
    - NO ABSOLUTE-WORD TELL: Never make a distractor wrong merely by inserting a sweeping/extreme qualifier — "only", "exclusively", "solely", "merely", "always", "never", "all", "none", "any", "every", "must", "guarantees" (and their equivalents in the question's language, e.g. Polish "wyłącznie / tylko / jedynie / zawsze / nigdy / wszystkie / żaden"). Such words are a dead giveaway that the option is false. A distractor must be wrong on its SUBSTANCE — believable to someone who did not study — not because it over-claims with an absolute. If an extreme qualifier appears at all, it must appear in correct AND incorrect options alike; the presence or absence of such a word must NEVER correlate with whether an option is correct, and correct options must not be systematically the softer or more hedged ones.
    ...
    - DON'T ECHO THE STEM: when the question already states the property being tested, each option must be just the bare candidate, NOT a sentence repeating the stem. (BAD stem "Which of these ARE basic risk-treatment methods?" with options "Risk avoidance is a basic risk-treatment method", "Risk transfer is a basic risk-treatment method". GOOD: options "Risk avoidance", "Risk transfer", "Risk acceptance", "Risk aggregation".) Never restate the question's predicate in every option; keep options as short as possible while staying unambiguous.
